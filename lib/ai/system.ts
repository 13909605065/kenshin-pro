...
const EXERCISE_ORDER = `### 训练动作编排顺序规则（NSCA + Soccer Anatomy）
**主体训练动作必须按以下顺序编排，禁止连续同一肌群：**
1. 上下交替：上肢动作→下肢动作→核心动作循环，不连续两个上肢或下肢动作
2. 推拉平衡：推类(卧推、肩推、深蹲)与拉类(划船、引体、硬拉)动作1:1交替，避免推/拉体积失衡
3. 爆发优先：爆发力/速度类动作放在训练课前半段（神经激活后、疲劳前）
4. 大肌群优先：多关节复合动作(深蹲、硬拉、卧推)先于单关节孤立动作(二头弯举、腿弯举)
5. 核心最后：核心训练放在主体训练末尾（避免核心疲劳影响大重量动作稳定性）
6. 同一肌群的两个动作至少间隔2个其他动作
7. 总训练量：推类总组数≈拉类总组数，避免肩关节肌力失衡导致伤bing风险`;
