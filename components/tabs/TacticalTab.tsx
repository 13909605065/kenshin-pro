  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-pitch-700/50 rounded-lg p-4">
        ...
        <span className="text-xs text-neon-pink bg-neon-pink/10 px-2 py-0.5 rounded mt-1 inline-block">
          {m.tactical_theme}
        </span>
      </div>
      ...
      {m.tactical_analysis && m.tactical_analysis.length > 0 && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">战术核心分析</h4>
      ...
      {m.formation_notes && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">阵型体系</h4>
      ...
      {(m.build_up_phase || m.midfield_transition || m.final_third || m.defensive_block) && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">比赛阶段分析</h4>
      ...
      {(m.pressing_triggers || m.defensive_shape) && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">压迫与防守组织</h4>
          <div className="space-y-2">
            {m.pressing_triggers && (
              <div className="bg-pitch-700/50 rounded-lg p-3">
                <span className="text-xs text-neon-pink font-bold">压迫触发信号</span>
                <p className="text-xs text-gray-300 mt-1 leading-relaxed">{m.pressing_triggers}</p>
              </div>
            )}
            {m.defensive_shape && (
              <div className="bg-pitch-700/50 rounded-lg p-3">
                <span className="text-xs text-neon-pink font-bold">防守阵型组织</span>
      ...
      {(m.attacking_patterns || m.counter_structure) && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">进攻与反击</h4>
          <div className="space-y-2">
      ...
      {m.transition_moments && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">攻守转换</h4>
          <div className="bg-pitch-700/50 rounded-lg p-3">
      ...
      {(m.set_piece_offense || m.set_piece_defense) && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">定位球战术</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {m.set_piece_offense && (
              <div className="bg-pitch-700/50 rounded-lg p-3 border-l-2 border-green-500">
            ...
            {m.set_piece_defense && (
              <div className="bg-pitch-700/50 rounded-lg p-3 border-l-2 border-red-500">
      ...
      {m.player_roles && m.player_roles.length > 0 && (
        <div>
          <h4 className="text-neon-pink text-sm font-bold mb-2">球员战术角色</h4>
          <div className="space-y-2">
            {m.player_roles.map((role, i) => (
              <div key={i} className="bg-pitch-700/50 rounded-lg p-3 flex items-start gap-3">
                <span className="text-neon-pink text-lg flex-shrink-0">&#9733;</span>
