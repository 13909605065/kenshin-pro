      ...
      <div className="max-w-3xl mx-auto px-4 py-4 space-y-4">
        ...
        {nextMatch && days !== null && (
          <div className="bg-[#d92525]/10 border border-[#d92525]/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] text-[#d92525] font-bold uppercase">下一场比赛</span>
              <span className="text-[10px] text-[#d92525]">{matchDayTrainingHint(days)}</span>
            </div>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { setEditing(null); setShowAdd(true); }}
            className="flex items-center gap-1 px-3 py-2 bg-[#d92525] text-white text-xs font-bold rounded-lg">
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map(m => (
              <div key={m.id}
                ...
                className="bg-[#1e1e1e] border border-[#333] rounded-xl p-4 hover:bg-[#222] transition cursor-pointer">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={"w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold " +
                      (m.status === "played" ? "bg-gray-700 text-gray-400" : "bg-[#d92525]/20 text-[#d92525]")}>
                      {m.location === "home" ? "主" : "客"}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    {m.status === "upcoming" && (
                      <button onClick={(e) => { e.stopPropagation(); router.push("/?matchOpponent=" + encodeURIComponent(m.opponent) + "&matchDate=" + m.date); }}
                        className="flex items-center gap-1 px-2 py-1 bg-[#d92525]/20 hover:bg-[#d92525]/30 border border-[#d92525]/30 rounded text-[10px] text-[#d92525] transition">
                        <Zap className="w-3 h-3" /> 生成训练
                      </button>
                    )}
                    {m.status === "played" && m.result && <span className="text-[11px] text-[#d92525] font-bold">{m.result}</span>}
...
function MatchModal({ match, onSave, onDelete, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#1e1e1e] border border-[#333] rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5 space-y-3" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 pt-2">
          <button onClick={() => onSave(form)} className="flex-1 bg-[#d92525] text-white font-bold py-2 rounded-lg text-sm">保存</button>
          {onDelete && (
