...
function CoachShareView({ modules }: { modules: TrainingModule[] }) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-4">
      <div className="bg-[#1e1e1e] rounded-xl p-4 border border-[#222]">
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-4">
      ...
      {posModule.cooldown?.map((c: any, i: number) => (
        <div key={i} className="bg-[#1e1e1e]/50 rounded p-3"><span className="text-white text-sm">🧊 {c.name}</span><span className="text-xs text-gray-400 ml-2">{c.duration}min</span></div>
      ))}
    </div>
  );
}
