...
const PHASE_PRESETS: Record<PhaseType, PhasePreset> = {
  offseason: {
    ...
    avgDuration: 75,
    intensityRange: '低~中低',
    weeklyLoadDesc: '力量3次/周（补短板重点）+ 外场2次/周（低强度有氧+个人技术）。周总负荷控制在赛季期60-70%。规则：禁用全队MD，仅加练小组模式，日历灰色锁定。',
  },
  preseason_build: {
    ...
    color: '#166534',
    goal: '体能储备 · 战术磨合',
    stateDescription: '全队逐步上量，建立有氧基础→力量储备→爆发力转化。热身赛检验阵容，确定主力框架。季前末尾1周自动生成黄色减量周（热身赛前降负荷）。',
    weeklyStrengthDays: 2,
    weeklyPitchDays: 4,
    avgDuration: 90,
    intensityRange: '中~高',
    weeklyLoadDesc: '力量2次/周（全身力量储备）+ 外场4次/周（SSG+战术演练+热身赛）。周负荷逐周递增，末尾1周自动减量。',
  },
  regular_season: {
    ...
    color: '#991b1b',
    goal: '以赛代练 · 维持状态',
    stateDescription: '一周一赛或双赛节奏。联赛日全力争胜，杯赛日轮换保持竞争力。训练围绕比赛日安排。关键战前整周可手动标记黄色减量周。',
    weeklyStrengthDays: 1,
    weeklyPitchDays: 4,
    avgDuration: 60,
    intensityRange: '中（保状态）',
    weeklyLoadDesc: '力量1-2次/周（维持力量，赛前2天禁止大重量下肢）+ 外场4次/周（MD-3战术→MD-1激活→MD比赛→MD+1恢复）。一周双赛时降至最低有效剂量。关键战前可手动标记减量。',
  },
  playoffs: {
    phaseType: 'playoffs',
    label: '附加赛',
                  ...
                  {isEditing && (
                    <div className="border-t border-[#222] p-3 bg-[#0d0d0d] rounded-b-lg">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        ...
                        <div className="col-span-2"><label className="text-[9px] text-gray-500 block mb-1">训练目标</label><div className="flex gap-1">{[
  {k:'strength',v:'力量'},{k:'power',v:'爆发力'},{k:'speed',v:'速度'},{k:'mas_endurance',v:'专项耐力'},{k:'agility',v:'协调灵敏'}
].map(g => <button key={g.k} onClick={() => updateDay(day.id, 'goal', g.k)} className={`px-2 py-1 rounded text-[9px] transition ${day.goal === g.k ? 'bg-[#992828] text-white' : 'bg-[#1a1a1a] text-gray-400 hover:text-white'}`}>{g.v}</button>)}</div></div>
