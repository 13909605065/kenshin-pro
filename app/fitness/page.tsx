...
const TESTS = [
  { id: "yoyo_ir1", name: "Yo-Yo IR1", unit: "m", elite: 2400, good: 2000, avg: 1600, low: 1200, desc: "20m折返跑，手机音频", higherIsBetter: true },
  { id: "sprint_30m", name: "30m冲刺", unit: "s", elite: 3.9, good: 4.1, avg: 4.3, low: 4.5, desc: "卷尺+秒表，站立起跑", higherIsBetter: false },
  { id: "sprint_10m", name: "10m冲刺", unit: "s", elite: 1.7, good: 1.8, avg: 1.9, low: 2.0, desc: "卷尺+秒表，反应启动", higherIsBetter: false },
  { id: "cmj", name: "CMJ 反向纵跳", unit: "cm", elite: 55, good: 48, avg: 40, low: 35, desc: "双手叉腰，全力跳", higherIsBetter: true },
  { id: "sj", name: "SJ 静蹲跳", unit: "cm", elite: 50, good: 42, avg: 35, low: 30, desc: "半蹲2s后全力跳", higherIsBetter: true },
  { id: "agility_505", name: "505敏捷", unit: "s", elite: 2.3, good: 2.5, avg: 2.7, low: 3.0, desc: "6锥桶，卷尺+秒表", higherIsBetter: false },
  { id: "weight", name: "体重", unit: "kg", elite: 0, good: 0, avg: 0, low: 0, desc: "体重秤（如有）", higherIsBetter: false },
  { id: "height", name: "身高", unit: "cm", elite: 0, good: 0, avg: 0, low: 0, desc: "卷尺/身高尺", higherIsBetter: false },
];

const TEST_NODES = [
  { value: "季前", label: "季前（到队第1周）" },
  { value: "季中", label: "季中（第9轮后）" },
  { value: "季末", label: "季末（预赛结束）" },
];

// Vo2max from Yo-Yo IR1 (Bangsbo et al., 2008)
function estimateVo2max(yoyoIR1: number): number {
  return Math.round((yoyoIR1 / 1000 * 0.0084 + 0.036) * 1000) / 10;
}
...
function classify(value: number, test: typeof TESTS[0]): "elite" | "good" | "average" | "low" {
  if (test.elite === 0) return "average"; // no norms for weight/height
...
export default function FitnessPage() {
  ...
  const [node, setNode] = useState("季前");
  ...
  const addResult = () => {
    ...
    const r: TestResult = {
      ...
      testId: activeTest.id, testName: activeTest.name, value: parseFloat(value), date, node,
  ...
  const handleCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (lines.length < 2) { setImportMsg("需标题行+数据行"); return; }
        ...
        const vi = headers.findIndex(h => h.includes("value")||h.includes("成绩")||h.includes("值"));
        const di = headers.findIndex(h => h.includes("date")||h.includes("日期"));
        const oi = headers.findIndex(h => h.includes("节点")||h.includes("node"));
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const nodeVal = oi >= 0 ? cols[oi] : "季前";
          const player = players.find(p => p.name === name);
          imported.push({
            id: `csv_${Date.now()}_${i}`, playerId: player?.id||`csv_${name}`, playerName: name,
            position: player?.position||"", testId: test.id, testName: test.name,
            value: val, date: dStr, node: nodeVal, level: classify(val, test),
          });
        }
  ...
  const exportCSV = () => {
    const h = "球员,位置,测试项目,成绩,日期,测试节点,等级";
    const rows = results.map(r => `${r.playerName},${r.position},${r.testName},${r.value},${r.date},${r.node},${r.level}`);
    const blob = new Blob([[h, ...rows].join("\n")], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = `体测_${new Date().toISOString().slice(0,10)}.csv`; a.click();
  };

  // Compute Yo-Yo → Vo2max for display
  const latestYoYo = useMemo(() => {
    const map: Record<string, number> = {};
    for (const r of results) {
      if (r.testId !== "yoyo_ir1") continue;
      if (!map[r.playerId] || r.date > (results.find(x => x.playerId === r.playerId && x.testId === "yoyo_ir1" && map[r.playerId])?.date || "")) {
        map[r.playerId] = r.value;
      }
    }
    return map;
  }, [results]);
    ...
    <div className="min-h-screen bg-[#121212] pb-20">
      <header className="sticky top-0 z-40 bg-[#121212]/90 backdrop-blur border-b border-[#222]">
        <div className="max-w-7xl mx-auto px-4 h-12 flex items-center gap-4">
          ...
          <span className="text-[10px] text-gray-600 hidden sm:inline">零硬件方案 · Yo-Yo+冲刺+CMJ+505</span>
        </div>
      </header>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              ...
              <div><label className="text-[9px] text-gray-500 block mb-1">球员</label>
                <select value={selectedPlayer} onChange={e => setSelectedPlayer(e.target.value)} className="w-full bg-[#0d0d0d] border border-[#333] rounded px-2 py-1.5 text-xs text-white">
                  ...
                  {players.map(p => <option key={p.id||p.name} value={p.id||p.name}>{p.name}</option>)}
                </select></div>
              <div><label className="text-[9px] text-gray-500 block mb-1">测试节点</label>
                <select value={node} onChange={e => setNode(e.target.value)} className="w-full bg-[#0d0d0d] border border-[#333] rounded px-2 py-1.5 text-xs text-white">
                  {TEST_NODES.map(n => <option key={n.value} value={n.value}>{n.label}</option>)}
                </select></div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead><tr className="border-b border-[#222] bg-[#0d0d0d]">
                  ...
                  {TESTS.filter(t => t.elite > 0).map(t => <th key={t.id} className="text-center p-3 text-gray-400 font-medium whitespace-nowrap">{t.name}<br/><span className="text-[9px] text-gray-600">({t.unit})</span></th>)}
                  <th className="text-center p-3 text-gray-400 font-medium">Vo2max<br/><span className="text-[9px] text-gray-600">推算</span></th>
                ...
                <tbody>
                  {players.slice(0,28).map(player => {
                    ...
                    const vo2 = latestYoYo[player.id];
                    return <tr key={player.id} className="border-b border-[#222] hover:bg-[#0d0d0d]">
                      <td className="p-3"><p className="text-white font-medium">{player.name}</p><p className="text-[10px] text-gray-500">{player.position}</p></td>
                      {perfTests.map(t => { const r = latest[t.id]; return <td key={t.id} className="text-center p-3">{r ? <span className={`inline-block px-2 py-0.5 rounded text-[10px] border ${LEVEL_STYLE[r.level]}`}>{r.value}{t.unit}</span> : <span className="text-gray-600">—</span>}</td>; })}
                      <td className="text-center p-3">{vo2 ? <span className="text-white font-medium">{estimateVo2max(vo2)}</span> : <span className="text-gray-600">—</span>}</td>
                  ...
                  {players.length===0 && <tr><td colSpan={9} className="p-6 text-center text-gray-500 text-xs">暂无球员，请先在花名册添加</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {view === "trends" && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-gray-500">季前→季中→季末变化</span>
            </div>
            {TESTS.filter(t => t.elite > 0).map(test => {
              ...
              return <div key={test.id} className="bg-[#1a1a1a] border border-[#222] rounded-xl p-4">
                ...
                {hist.map(r => <div key={r.id} className="flex items-center gap-3 text-[10px] py-1"><span className="text-gray-500 w-16">{r.date}</span><span className="text-gray-500 w-12">{r.node}</span><span className="text-white font-medium w-16 truncate">{r.playerName}</span><span className={`px-1.5 py-0.5 rounded border text-[9px] ${LEVEL_STYLE[r.level]}`}>{r.value}{test.unit}</span><button onClick={()=>deleteResult(r.id)} className="text-gray-600 hover:text-red-500"><Trash2 className="w-3 h-3"/></button></div>)}
              </div>;
            })}
          </div>
        )}
